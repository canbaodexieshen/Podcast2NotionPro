...
#################################################################################################################
    def upload_file_to_notion(self, file_path: str) -> str:
        """
        先把 file_path 上传到你的图床（utils.upload_image），
        返回一个可以 Notion 外链引用的 URL。
        """
        filename = os.path.basename(file_path)
        # 这里只有调用上传接口，具体取决你原来 upload_image 如何实现
        return upload_image("file", filename, file_path)["url"]

    def append_transcription_and_mindmap(self, page_id: str, transcript: str, mindmap_url: str):
        properties = {
            "转写": {
                "rich_text": [{"type": "text", "text": {"content": transcript}}]
            },
            "思维导图": {
                "files": [{"name": "mindmap.xmind", "external": {"url": mindmap_url}}]
            },
        }
