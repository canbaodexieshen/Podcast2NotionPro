...
if __name__ == "__main__":
    ...
    # —— 新增：取出刚写入的所有记录，依次处理转写 & 思维导图 —— #
    db_id = os.getenv("NOTION_DATABASE_ID")
    all_pages = notion_helper.query_all(database_id=db_id, filter={"property":"状态","select":{"equals":"在听"}})

    for page in all_pages:
        page_id = page["id"]
        # 拿到音频链接字段
        url_prop = page["properties"]["音频"]["rich_text"][0]["text"]["content"]
        print(f"正在处理转写: {page_id} -> {url_prop}")

        # 1) 转写
        transcript = transcribe_with_tingshu(url_prop)
        print(f"转写完成，长度 {len(transcript)} 字")

        # 2) 生成思维导图（.xmind）
        local_map = os.path.join("output_maps", f"{page_id}.xmind")
        os.makedirs(os.path.dirname(local_map), exist_ok=True)
        title = page["properties"]["标题"]["title"][0]["text"]["content"]
        map_path = generate_mindmap(transcript, local_map, title)

        # 3) 上传并写回 Notion
        mindmap_url = notion_helper.upload_file_to_notion(map_path)
        notion_helper.append_transcription_and_mindmap(page_id, transcript, mindmap_url)
        print(f"已更新 Notion，页面 {page_id} 完成转写和思维导图。")
