...
from utils import download_image, upload_image  # 你已有

#####################################################################################################################

TINGYI_API_URL = os.getenv("TINGYI_API_URL")  # e.g. https://openapi.tingyi.aliyun.com/v1
TINGYI_API_KEY = os.getenv("TINGYI_API_KEY")


def transcribe_with_tingshu(audio_url: str, wait_interval: int = 5, timeout: int = 600) -> str:
    """
    提交音频 URL 到通义听悟，返回完整转写文本。
    """
    headers = {
        "Authorization": f"Bearer {TINGYI_API_KEY}",
        "Content-Type": "application/json"
    }
    # 1) 提交任务
    resp = requests.post(
        f"{TINGYI_API_URL}/transcriptions",
        headers=headers,
        json={"audio_url": audio_url, "language": "zh_CN"}
    )
    ...
    while True:
        status_resp = requests.get(
            f"{TINGYI_API_URL}/transcriptions/{task_id}",
            headers=headers
        )
...
def generate_mindmap(transcript: str, output_path: str, title: str = "思维导图") -> str:
    """
    将 transcript（长文本）拆分为若干句子，生成一个简单的 XMind 思维导图，保存在 output_path。
    """
    ...
    root.setTitle(title)
    # 拆句（非常简单的按句号、问号、感叹号）
    for idx, sent in enumerate([s.strip() for s in transcript.replace("？", "。").replace("！", "。").split("。")]):
        if not sent:
            continue
        sub = root.addSubTopic()
        sub.setTitle(f"{idx + 1}. {sent[:50]}{'...' if len(sent) > 50 else ''}")
    wb.save(output_path)
    return output_path
